...
- 🎯 Entry points to execute your program using `python -m <cuddly_octo_palm_tree>` or `$ cuddly_octo_palm_tree` with basic CLI argument parsing.
...
---
# cuddly_octo_palm_tree

[![codecov](https://codecov.io/gh/brainwarr/cuddly-octo-palm-tree/branch/main/graph/badge.svg?token=cuddly-octo-palm-tree_token_here)](https://codecov.io/gh/brainwarr/cuddly-octo-palm-tree)
[![CI](https://github.com/brainwarr/cuddly-octo-palm-tree/actions/workflows/main.yml/badge.svg)](https://github.com/brainwarr/cuddly-octo-palm-tree/actions/workflows/main.yml)

Awesome cuddly_octo_palm_tree created by brainwarr

## Install

from source
```bash
git clone https://github.com/brainwarr/cuddly-octo-palm-tree cuddly_octo_palm_tree
cd cuddly_octo_palm_tree
make install
```
...
```bash
pip install cuddly_octo_palm_tree
```

## Executing

```bash
$ cuddly_octo_palm_tree run --port 8080
```

or

```bash
python -m cuddly_octo_palm_tree run --port 8080
```

or

```bash
$ uvicorn cuddly_octo_palm_tree:app
```

## CLI

```bash
❯ cuddly_octo_palm_tree --help
Usage: cuddly_octo_palm_tree [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ cuddly_octo_palm_tree create-user --help
Usage: cuddly_octo_palm_tree create-user [OPTIONS] USERNAME PASSWORD
...
```bash
cuddly_octo_palm_tree create-user admin admin --superuser
```
...
Run with `cuddly_octo_palm_tree run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `cuddly_octo_palm_tree create-user`.
...
-----------------------------------------------------
cuddly_octo_palm_tree/__init__.py              4      0   100%
cuddly_octo_palm_tree/app.py                  16      1    94%
cuddly_octo_palm_tree/cli.py                  21      0   100%
cuddly_octo_palm_tree/config.py                5      0   100%
cuddly_octo_palm_tree/db.py                   10      0   100%
cuddly_octo_palm_tree/models/__init__.py       0      0   100%
cuddly_octo_palm_tree/models/content.py       47      1    98%
cuddly_octo_palm_tree/routes/__init__.py      11      0   100%
cuddly_octo_palm_tree/routes/content.py       52     25    52%
cuddly_octo_palm_tree/routes/security.py      15      1    93%
cuddly_octo_palm_tree/routes/user.py          52     26    50%
cuddly_octo_palm_tree/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in cuddly_octo_palm_tree/default.toml.
